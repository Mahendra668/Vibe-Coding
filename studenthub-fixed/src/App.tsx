...
function AppContent() {
  ...
  // FIX 4: State variable to switch between login and register
  ...
  if (!user) {
    return (
      <Suspense fallback={<LoadingSpinner className="min-h-screen" />}>
        {/* FIX 4: authMode drives the form type; onToggle flips between login/register */}
        <AuthForm
          type={authMode}
      ...
      <Suspense fallback={<LoadingSpinner className="min-h-[400px]" />}>
        <ErrorBoundary>
          <Routes>
            ...
            {/* FIX 6: SearchResults now reads query from URL itself */}
