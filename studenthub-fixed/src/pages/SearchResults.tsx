...
// FIX 6: Read query from URL search params; use useNavigate for navigation
import { useSearchParams, useNavigate } from 'react-router-dom';

// FIX 6: No longer accepts props — reads everything from URL and router
export default function SearchResults() {
  // FIX 6: Get query from ?q= URL parameter
  ...
  // FIX 6: Navigate to the correct tool route
  ...
  if (!query) {
    return (
      <div className="bg-[#121214] border border-white/5 rounded-3xl p-16 text-center opacity-50">
        ...
        <h3 className="text-2xl font-bold mb-2">No search query</h3>
        <p className="text-zinc-500">Use the search bar above to find tools, history, or resources.</p>
      </div>
    );
  }
            ...
            <div className="space-y-3">
              {results.resources.map((res: any, idx: number) => (
                <motion.div
                  key={idx}
                  whileHover={{ scale: 1.02 }}
                  className="bg-[#121214] border border-white/5 rounded-2xl p-5 hover:border-amber-500/30 transition-all group"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
